...
async function parseCommand(text, msg, isTelegram = true) {
  ...
  if (text.match(/^\/(info|help)$/)) {
    ...
    sendPlatformsCalls(user, platform, msg, isTelegram);
  } else if (text.match(/^\/(solo|duo|squad)(s[3-9])?(pc|xbox|ps4)?(\s.+)?$/)) {
    ...
    platform = platform.replace(/s[3-9]/i, '');
  ...
  } else if (text.match(/^\/(season|s)[3-9](pc|xbox|ps4)?(\s.+)?$/)) {
...
function getModeInfo(mode) {
  ...
  const season = range(3, constants.CURR_SEASON + 1).includes(lastChar) ? lastChar : '';
